...
public class BuilderWindow : EditorWindow
{
    ...
    private UnityEngine.Object GetKitObject(string path) {
        ...
        if (!Utils.ALLOWED_KIT_TYPES.Contains(obj.GetType()))
        {
            Debug.LogWarning($"Asset at path {path} isn't a valid kit bundle object type, it is {obj.GetType().Name}.  Allowed types are: {string.Join(", ", Utils.ALLOWED_KIT_TYPES.Select(x => x.Name))}");
            return null;
        }
        return obj;
    } 
    ...
    private void BuildAssetBundles() {
        ...
        if (EditorUtility.DisplayDialog("Are you sure?", "This will clear any files in the output folder.", "Continue", "Cancel")) {
            ...
            if (mode == BanterBuilderBundleMode.Kit) {
                AddStatus("Writing kit items to " + assetBundleDirectory + "/kit_items.txt.");
                File.WriteAllText(assetBundleDirectory + "/kit_items.txt", String.Join("\n", kitObjectList.Select(x => x.path.ToLower()).ToArray()));
            }
