...
namespace Katas.Experimental
{
        ...
        ISubmitHandler, ICancelHandler, IMoveHandler, IScrollHandler, ISelectHandler, IDeselectHandler, IDragHandler
    {
        ...
        public void RebuildPanel ()
        {
            ...
            _uiDocument.rootVisualElement.RegisterCallback<FocusOutEvent>( e => {
              //  Controls.ToggleKeyboardControls(true);
            });
            _uiDocument.rootVisualElement.RegisterCallback<FocusInEvent>(async e => {
                //await new WaitForSeconds(0.1f);
                var focused = _uiDocument.rootVisualElement.focusController.focusedElement;
                if(focused?.GetType() == typeof(TextField)) {
                //    Controls.ToggleKeyboardControls(false);
                }
            });
        ...
        protected void TransformPointerEventForUIToolkit (PointerEventData eventData)
        {
            ...
            if (!_eventsProcessedInThisFrame.Contains(eventKey))
            {
                ...
                if (eventCamera != null)
                {
                    ...
                    UnityEngine.Plane panelPlane = new UnityEngine.Plane(transform.forward, transform.position);
