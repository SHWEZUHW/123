...
namespace UnityEngine.XR.Interaction.Toolkit.UI
{
    ...
    [AddComponentMenu("Event/World Tracked Physics Raycaster", 11)]
    public class WorldTrackedPhysicsRaycaster : BaseRaycaster
    {
        /// <summary>
        ...
        public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList) {
            if (eventData is ExtendedPointerEventData trackedEventData) {
        ...
        void PerformRaycasts(ExtendedPointerEventData eventData, List<RaycastResult> resultAppendList) {
            ...
            Vector3 forwardVector = eventData.trackedDeviceOrientation * Vector3.forward;
            Vector3 from = eventData.trackedDevicePosition;
            PerformRaycast(from, forwardVector, m_EventMask, currentEventCamera, resultAppendList);
            // var rayPoints = eventData.GetRayPoints();
        ...
        bool PerformRaycast(Vector3 from, Vector3 forward, LayerMask layerMask, Camera currentEventCamera, List<RaycastResult> resultAppendList)
        {

            var hitSomething = false;

            var rayDistance = 20;//Vector3.Distance(to, from);
            var ray = new Ray(from, forward * rayDistance);
            ...
            Sort(m_RaycastResultsCache, m_RaycastHitComparer);
